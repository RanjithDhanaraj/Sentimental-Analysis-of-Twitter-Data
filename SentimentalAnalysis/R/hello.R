...
api_key <- "dadJ8d4OvEuqHIw5og4SRfISY"
api_secret <- "F7coOOv3Ld6WgDf0YaY5aafeyMaZNMa8FItGkL13oyhkgRFe00"
access_token <- "870286501061419017-bhpfsebHSDdWxhyA46JaLxHngEUoV9m"
access_token_secret <- "srkPYhrY9TLk4szJH7fqUwgjJLjsDxthngQ9CSn9YfH3a"

setup_twitter_oauth(api_key,api_secret,access_token,access_token_secret)

pos.words <- read.delim("C:\\Users\\USER1\\Documents\\SentimentalAnalysis\\Data\\Positive.txt", header=FALSE, stringsAsFactors=FALSE)
pos.words <- pos.words$V1
pos.words <- unlist(lapply(pos.words, function(x) { str_split(x, "\n") }))
neg.words <- read.delim("C:\\Users\\USER1\\Documents\\SentimentalAnalysis\\Data\\Negative.txt", header=FALSE, stringsAsFactors=FALSE)
neg.words <- neg.words$V1
neg.words <- unlist(lapply(neg.words, function(x) { str_split(x, "\n") }))
